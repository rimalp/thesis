...
% have to have run 'converter' before you run this file.
...
makebatches;
[numcases numdims numbatches]=size(batchdata); % batchdata is 3d matrix, #digits x data per digit x batch number
N=numcases; % number of cases per batch
...
%%%%%%%%% ==== MODIFY THIS (SEPARATE) TO DO THE CLASSIFICATION AFTER TRAINING
err=0;
err_cr=0;
counter=0;
%load the test data
[testnumcases testnumdims testnumbatches]=size(testbatchdata);
N=testnumcases; %test cases per batch
for batch = 1:testnumbatches
  data = [testbatchdata(:,:,batch)]; % get all the data for that batch
  target = [testbatchtargets(:,:,batch)]; % get all the labels for that batch fo data
  data = [data ones(N,1)];
  w1probs = 1./(1 + exp(-data*w1)); w1probs = [w1probs  ones(N,1)];
  w2probs = 1./(1 + exp(-w1probs*w2)); w2probs = [w2probs ones(N,1)];
  w3probs = 1./(1 + exp(-w2probs*w3)); w3probs = [w3probs  ones(N,1)];
  ...
  [I J]=max(targetout,[],2);
  [I1 J1]=max(target,[],2);
  % rimalp: J vs J1 -> matches the two, increase the count
  
  if(epoch == testnumbatches) 
      fprintf('Looking for: %d, found: %d', J, J1);
  end
  counter=counter+length(find(J==J1)); %% rimalp: Checks for match here
  err_cr = err_cr- sum(sum( target(:,1:end).*log(targetout)));
end
