...
% Work with test files first ===============	reading and saving test
% data for each digit in files first
...
for i=1:10,
  ...
  rawimages = reshape(rawimages,28*28,n); %50 images of 28x28
...
for d=0:9,
  fclose(Df{d+1});
  D = load(['test' num2str(d) '.ascii'],'-ascii'); %raw data
  fprintf('%5d Digits of class %d\n',size(D,1),d);
  save(['test' num2str(d) '.mat'],'D','-mat');
end;


% Work with trainig files second  ====================== read and save
...
fprintf(1,'Starting to convert Training MNIST images (or a part of it)\n'); 
n = 50;%1000;%rimalp
